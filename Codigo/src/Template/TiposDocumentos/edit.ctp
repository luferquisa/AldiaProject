<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $tiposDocumento->codigo],
                ['confirm' => __('Are you sure you want to delete # {0}?', $tiposDocumento->codigo)]
            )
        ?></li>
        <li><?= $this->Html->link(__('List Tipos Documentos'), ['action' => 'index']) ?></li>
    </ul>
</nav>
<div class="tiposDocumentos form large-9 medium-8 columns content">
    <?= $this->Form->create($tiposDocumento) ?>
    <fieldset>
        <legend><?= __('Edit Tipos Documento') ?></legend>
        <?php
            echo $this->Form->input('id');
            echo $this->Form->input('grupo_documento');
            echo $this->Form->input('nombre_documento');
            echo $this->Form->input('estado');
        ?>
    </fieldset>
    <?= $this->Form->button(__('Submit')) ?>
    <?= $this->Form->end() ?>
</div>
